...
def fnGetStationaryTimeSeries(pDf,pLstCols=None,pHorizon=1):
    ...
    for fld in lstFlds:
        ...
        pDf[fld] =pDf[fld] -pDf[fld].shift(periods=pHorizon)
...
def fnPlotChart(pBenchmark=None, pPredictions=None,pActual=None, pStrStock="", pStrTitle="", pBenchLabel=""):
    ...
    plt.plot_date(pBenchmark['Date'], pBenchmark['Adj Close'], 'b-',label=benchLabel)
...
def fnGetModelMultiLayer(nLayers=0,numNeurons=0, nDropout=0,nEpochs=3000,nLearnRate=.00001): # neuronsL1=0,neuronsL2=0,neuronsL3=0):
    model = Sequential()
    blnStateful=True
...
def fnGridSearchModel(trainX, trainY):
    ...
    model = KerasRegressor(build_fn=fnGetModelMultiLayer, nb_epoch=1000, batch_size=batch_size,
                            verbose=2)
    ...
    #nLearnRate =[.000000001,.00000001,.0000001,.000008 ,.0001 ]
...
def fnGetModel(blnStateful=True, pLook_Back=7, nEpochs =3000):
    global learn_rate
    learn_rate=0.007714426
    model = Sequential()
   ...
   # {'input': 1, 'hidden1': 48, 'hidden2': 16, 'hidden4': 105, 'hidden3': 16, 'output': 1}
    ...
    #decreasing learning rate increases vol
    model =fnGetModelMultiLayer(nLayers=1,numNeurons=numFeatures*2, nDropout=0.47,nLearnRate=  3e-04, nEpochs=nEpochs)
...
def fnComputePredictedPrices(pBenchMark, pOriginalDF, pPredictions):
	...
	#predicted values are 1 day differences in log of prices
        ...
        forecastDF['BenchmarkAdjClose'] =forecastDF['LogAdjClose'] +pBenchMark['Adj Close']
        ...
        forecastDF['PredictedAdjClose']  =forecastDF['PredictedAdjClose'].shift(horizon)
        forecastDF['BenchmarkAdjClose']=forecastDF['BenchmarkAdjClose'].shift(horizon)
...
def fnRunVXX(pBlnGridSearch =False):
    ...
    lstrStock="VXX"
    fnMain(lstrStock,lStartDateTrain,lEndDateTrain, lStartDateTest,  lEndDateTest,pBlnGridSearch)


def fnRunOIL(pBlnGridSearch =False):
...
def fnRunSPY2015(pBlnGridSearch =False):
...
def fnMain(pSymbol="", pStartDateTrain=None, pEndDateTrain=None, pStartDateTest=None,
           pEndDateTest=None, pModelFile=None,pBlnGridSearch=False):
        '''
        ...
        batch_size = 3*look_back #*2#*5 #higher batch sz gives better results for SPY.
        ...
        if pBlnGridSearch:
            ...
            for lr in np.arange(.000001,.009,0.001285571):
                learn_rate=lr 
        ...
        if blnLoadModel:
                ...
                model = load_model(lModelPath+'KerasStockModel.h5')
                        ...
                        model=fnGetModel(False, look_back,nEpochs)
        ...
        if True:
                ...
                fnPlotChart(dfPredictions[['BenchmarkAdjClose']], dfPredictions[['PredictedAdjClose']],\
                    dfPredictions[['Adj Close']], lstrStock,"Predictions On Test Data")
